...
export default function HighPage() {
  ...
  const queryKey = ["mengla", "high", primaryCatId, dateType, timest];

  const query = useQuery({
    queryKey,
    queryFn: () => queryMengla(buildQueryParams("high", primaryCatId, period, timest)),
  ...
  const list = useRankList(pickPayload(query.data), "highList");
